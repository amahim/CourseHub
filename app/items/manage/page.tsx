      ...
      <div className="bg-gradient-to-r from-primary-600 to-primary-800 dark:from-primary-900 dark:to-gray-900 text-white py-12">
        <div className="container-custom">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <Link
              href="/items/add"
              className="inline-flex items-center gap-2 px-5 py-2.5 bg-white text-primary-700 font-semibold rounded-xl hover:bg-primary-50 transition-colors w-fit"
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-8">
          {[
            ...
            },
            {
              ...
              value: courses.length
                ? (
                    courses.reduce((s, c) => s + (c.rating || 0), 0) /
                    courses.length
                  ).toFixed(1)
                : "—",
                ...
                <thead className="bg-gray-50 dark:bg-gray-900 border-b border-gray-100 dark:border-gray-700">
                  <tr>
                    {[
                      ...
                      <th
                        key={h}
                        className={`px-5 py-3.5 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider ${h === "Actions" ? "text-right" : ""}`}
                      >
                        {h}
                      </th>
                    ))}
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                  {filtered.map((course) => (
                    ...
                    >
                      ...
                      <td className="px-5 py-4">
                        <span className="px-2.5 py-0.5 text-xs font-medium rounded-full bg-primary-100 dark:bg-primary-900/30 text-primary-700 dark:text-primary-400">
                          {course.category}
                        </span>
                      </td>
                      <td className="px-5 py-4">
                        <span
                          className={`text-xs font-medium ${course.level === "Beginner" ? "text-green-600 dark:text-green-400" : course.level === "Advanced" ? "text-red-500 dark:text-red-400" : "text-yellow-600 dark:text-yellow-400"}`}
                        >
                          {course.level || "—"}
                        </span>
                      </td>
              ...
              {filtered.map((course) => (
                ...
                >
                  ...
                  <div className="flex gap-2">
                    <Link
                      href={`/items/${course.id}`}
                      className="flex-1 btn-secondary text-center py-2 text-sm dark:border-gray-600 dark:text-gray-300"
                    >
